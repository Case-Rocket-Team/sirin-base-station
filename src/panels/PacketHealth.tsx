...
export default function PacketHealth({ source, status, packet, expectedPacketsPerSecond, staleTimeoutMs }: Props) {
  ...
  return (
    <aside className="rounded-[20px] border border-white/10 bg-slate-950/55 p-2 backdrop-blur min-w-[1rem]">
      <p className="font-mono text-[10px] uppercase tracking-[0.35em] text-cyan-200/75 truncate">{source.toUpperCase()} Health</p>
      <div className="mt-1 grid grid-cols-2 gap-1 min-w-0">
...
function MetricCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-white/8 bg-black/20 p-1.5 min-w-0 overflow-hidden">
      <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-slate-400 truncate">{label}</p>
