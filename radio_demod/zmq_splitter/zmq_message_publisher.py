...
class zmq_splitter(gr.top_block):
    def __init__(
        ...
        gr.top_block.__init__(self,"HackRF RX 2x ZMQ PUB", catch_exceptions=True)
...
def run(config):
    tb = zmq_splitter(
        center_freq=config.get("freq", 434.5e6),
        samp_rate=config.get("samp_rate", 2_000_000),
        bw=config.get("bw", 125_000),
        gain=config.get("gain", 20),
        addr0=config.get("addr0", "tcp://127.0.0.1:5555"),
        addr1=config.get("addr1", "tcp://127.0.0.1:5556"),
        hwm=config.get("hwm", 256),
    ...
    print(f"  #0: {config.get('addr0', 'tcp://127.0.0.1:5555')}")
    print(f"  #1: {config.get('addr1', 'tcp://127.0.0.1:5556')}")
    signal.pause()
